...
public static class GitProviderGatewayExtensions
{
    public static async Task<List<string>> GetRecursive(
    ...
    static async Task GetRecursive(
        IGitProviderGateway gateway,
        string sourceOwner,
        string sourceRepository,
        string? path,
    ...
    )
    {
        foreach (
            var content in (
                await gateway.TreeFrom(
                    new(sourceOwner, sourceRepository, TreeEntryTargetType.Tree, branch, path),
                    true
                )
            )!
                .Item2
